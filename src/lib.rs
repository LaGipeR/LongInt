...
mod tests {
    ...
    fn test_not() {
        let a = LongInt::new();
        let b = !a;
        assert_eq!(b.blocks, vec![u32::MAX]);

        let b = !b;
        assert_eq!(b.blocks, vec![0]);

        let mut a = LongInt::from_hex("c");
        let b = !(&a);
        assert_eq!(b.blocks, vec![0b11111111111111111111111111110011]);

        a.setHex("ffffffffffffffff0000000000000000");
        let c = !(&a);
        assert_eq!(c.getHex(), "ffffffffffffffff");
        ...
        assert_eq!(
            b.blocks,
            vec![
                0b11101100100111101111000010001010,
                0b01101010101010101101000101100101
            ]
        );
        assert_eq!(
            a.blocks,
